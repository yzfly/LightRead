...
- **专业 PDF 阅读**：MuPDF 清晰渲染，默认适高，支持翻页、连续滚动、双页、自动阅读和听书。
...
- **连续阅读与清晰渲染**：论文默认连续滚动；页面由 MuPDF 按设备像素比绘制，PDFium 提供文字选择、链接、目录、批注和文本分析。
...
- PDF 页面使用与 SumatraPDF 同源的 MuPDF 渲染内核；文字选择、目录、链接和标注由 PDFium 提供。
...
核心结构：Vue 3 + TypeScript 负责界面，foliate-js 负责电子书，MuPDF 负责 PDF 可见页面，PDFium 负责 PDF 交互和文本几何，Tauri 2 / Rust 提供桌面文件系统、SQLite、网络和本地能力。网页版可以通过 `npm run build` 生成，但桌面版拥有完整的文件关联、Calibre、本地离线语音和无跨域网络能力。
