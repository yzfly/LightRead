...
5. **设置存储与备份**：进入“设置 → 数据”，可以更改书库位置、导入 / 导出开放的 OKF 藏书包，或配置 WebDAV。
...
- 翻页 / 滚动双模式，宽屏双栏，触屏点击和滑动翻页；翻页模式下上滑 / 下滑也可翻页，滚动模式则整本上下连续滑动。
...
- PDF 默认连续滚动并适宽显示，可切换适高、单页、对页和书籍视图。翻页模式下页面高于视口时先在页内上下滑动，滑到页尾再继续（滚轮、触屏或键盘）即翻到下一页，页首上翻则回到上一页页尾。
